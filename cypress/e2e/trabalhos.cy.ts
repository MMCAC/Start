...
describe("Testes sobre o endpoint /trabalhos", () => {
    before(() => {
        ...
        cy.fixture("trabalho_com_codigo_invalido").as(
    ...
    test("não deve salvar um trabalho com título vazio ou nulo", function () {
        ...
        cy.request(requestOptions).then((res) => {
            ...
            const {mensagemDeErro} = res.body;
            expect(mensagemDeErro[0]).to.equal("O título do trabalho não pode ser vazio");
        })
    });
    ...
    test("não deve salvar um trabalho com menos de 2 autores", function () {
        ...
        cy.request(requestOptions).then((res) => {
            expect(res.status).to.equal(400);
            const {mensagemDeErro} = res.body;
            expect(mensagemDeErro[0]).to.equal("O trabalho deve conter entre 2 e 7 autores")
        });
    });
    ...
    test("não deve salvar um trabalho com mais de 7 autores", function () {
      ...
      cy.request(requestOptions).then((res) => {
        expect(res.status).to.equal(400);
        const { mensagemDeErro } = res.body;
        expect(mensagemDeErro[0]).to.equal(
    ...
    test("não deve salvar um trabalho com uma área inválida", function () {
      ...
      cy.request(requestOptions).then((res) => {
        ...
        const { mensagemDeErro } = res.body;
        expect(mensagemDeErro[0]).to.equal(
          "A área do trabalho deve ser uma entre as opções: CAE, CET, CBS, CHCSA e MDIS.",
        );
      });
    });

    test("não deve salvar um trabalho com uma código inválido", function () {
      requestOptions.body = this.trabalhoComCodigoInvalido;
      cy.request(requestOptions).then((res) => {
        expect(res.status).to.equal(400);
        const { mensagemDeErro } = res.body;
        expect(mensagemDeErro[0]).to.equal(
          "O código do trabalho deve ser composto pelo código da área seguido de dois dígitos",
        );
      });
    });
    ...
    test("não deve salvar um trabalho com autor com nome inválido", function () {
      ...
      cy.request(requestOptions).then((res) => {
        ...
        const { mensagemDeErro } = res.body;
        expect(mensagemDeErro[0]).to.equal(
          "Os nomes do autores devem conter nome e sobrenome",
        );
      });
    });
    ...
    test("não deve salvar um trabalho com autor com gênero inválido", function () {
      ...
      cy.request(requestOptions).then((res) => {
        ...
        const { mensagemDeErro } = res.body;
        expect(mensagemDeErro[0]).to.equal(
          "Os gêneros do autores devem sem uma dentre as opções: M ou F.",
        );
      });
    });
